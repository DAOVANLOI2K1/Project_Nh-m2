...
class Guest extends Component {
    constructor(props) {
        ...
        this.state = {
            "data": [],
            "urlAPIGuests": "https://localhost:5001/api/v1/Guests",
            "search": "",
            "pagenumber": 1,
            "rowsofpage": 10,
            "remainingRows": 10,
    ...
    handlePaging(number, search) {
        ...
        if (number !== null) {
            let url = this.state.urlAPIGuests + "?pagenumber=" + number +
    ...
    getTotalData(search, number) {
        let url = this.state.urlAPIGuests + "?pagenumber=1&rowsofpage=-1";
        if (search) {
            url += "&search=" + search;
        }
        let config = this.getConfigToken();
    ...
    componentDidMount = (url = this.state.urlAPIGuests + "?pagenumber=1&rowsofpage=" + this.state.rowsofpage, search = "", number = 1) => {
        this.getData(url);
    ...
    deleteGuest(url) {
        ...
        axios.delete(url, config)
            .then((response) => {
                url = this.state.urlAPIGuests + "?pagenumber=" + this.state.activePage + "&rowsofpage=" + this.state.rowsofpage;
                if ($(".search-input").val()) {
...
=======
    handleDelete() {
        ...
        let url = this.state.urlAPIGuests + "/" + khid;
        if (khid != null) {
>>>>>>> loihoangkim
            return (
                <div>
...
>>>>>>> loihoangkim
    ...
    updateGuest(guest) {
        let config = this.getConfigToken();
        let url = this.state.urlAPIGuests;
    ...
    handleUpdate(guest) {
        ...
        if (this.validateFormUpdate(guest)) {
            return (
                <div>
                    {Swal.fire({
                        title: 'Bạn có chắc chắn sửa thông tin khách hàng?',
...
>>>>>>> loihoangkim
        ...
        if (this.validateFormCreate(guest)) {
            return (
                <div>
                    {Swal.fire({
                        title: 'Bạn có chắc chắn thêm khách hàng này?',
                        showDenyButton: true,
                        showCancelButton: true,
                                ...
                                <div className="infor_form">
                                    <div className="infor_form-left">
                                        ...
                                        <input type="Text" className="ms-input form_input" id="createBy" placeholder="Tên người tạo" defaultValue={localStorage.getItem("EmployeeName")}
                                            onChange={(e) => $(e.target).removeClass("required")} disabled />
...
=======
                            ...
                            <div className="update flex_center" commandtype="update" onClick={(e) => {
                                this.setState({ showFormUpdate: true });
