...
class Guest extends Component{
    ...
    initEventPaging(target){
        $(".page_navigate").find(".pagging_number").removeClass("active");
        $(target).addClass("active");
    }
    componentDidMount = (url = "https://localhost:5001/api/v1/Guests?pagenumber=1&rowsofpage=50") => {
        this.getData(url);
      }
      // Format ngày tháng
    formatDate = dateSrc => {
    ...
    renderItem(){
      ...
      handleSearch(search){
        let url = "https://localhost:5001/api/v1/Guests?pagenumber=1&rowsofpage=50" + search;
        this.componentDidMount(url);
      }
      handle(number, search){
        // let number = $(".page_navigate .active").attr("value");
        if(number !== null){
            let url = "https://localhost:5001/api/v1/Guests?pagenumber=" + (number + 1) + 
        ...
        return(
                <div className="page_right-content">
                    ...
                    <div className="section2 flex_center" id="show_option">
                        ...
                        <div className="search_option">
                            <input type="text" className="search_input ms-input" option_name="Search" placeholder="Tìm kiếm theo Tên khách hàng" onChange={(e) => {
                                this.handle($(".page_navigate .active").attr("value"), e.target.value)}} />
                    ...
                    <div className="page_navigate">
                        ...
                        this.handle(e.target.value, $(".page_navigate .active").attr("value"))}}>1</li>
                        <li className="pagging_number" value={1}
                        onClick = {(e) => {
                        this.initEventPaging(e.target)
                        this.handle(e.target.value, $(".page_navigate .active").attr("value"))}}>2</li>
                        <li className="pagging_number" value={2}
                        onClick = {(e) => {
                        this.initEventPaging(e.target)
                        this.handle(e.target.value, $(".page_navigate .active").attr("value"))}}>3</li>
                        <li className="pagging_number" value={3}
                        onClick = {(e) => {
                        this.initEventPaging(e.target)
                        this.handle(e.target.value, $(".page_navigate .active").attr("value"))}}>4</li>
                        <li className="pagging_number" value={4}
                        onClick = {(e) => {
                        this.initEventPaging(e.target)
                        this.handle(e.target.value, $(".page_navigate .active").attr("value"))}}>5</li>
                        </ul>
