...
class Guest extends Component{
    ...
    validateFormUpdate(guest){
       ...
       if (guest.sdt === "") {
           errorOfSDT = "Số điện thoại không được bỏ trống!\n";
       }
        if (!numbersOnly.test(guest.sdt)) {
            errorOfSDT = 'Số điện thoại chỉ chứa số';
        }
        else if (guest.sdt.length > 20) {
            errorOfSDT = "Số điện thoại không vượt quá 20 ký tự!\n";
        }
        else if(!firstPhoneNumber.test(guest.sdt)){
