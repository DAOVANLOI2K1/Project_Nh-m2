...
class Bill extends Component {
    ...
    renderbuttonThanhToan(status){
        if(status === "Chưa thanh toán"){
            return(<td><button type="button" className="btn btn-success btn-sm">Thanh Toán</button></td>);
        }
    }
    renderItem() {
        return this.state.data.map((item) => {
            return (
                <tr>
                    ...
                    {this.renderbuttonThanhToan(item.trangThai)}
                </tr>
            );
        });
    }
