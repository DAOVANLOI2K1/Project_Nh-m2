...
class Guest extends Component{
    ...
    deleteGuest(url){
        axios.delete(url)
        .then((response) => {
            let url = this.state.defaultUrl + "?pagenumber=" + this.state.activePage + "&rowsofpage=" + this.state.rowsofpage; 
            if($(".search-input").val()){
                url += "&search=" + $(".search-input").val();
            }
            this.componentDidMount(url , this.state.activePage, $(".search-input").val());
        });
    }
    handleDelete(){
        let khid = $("#gridGuest").find("tr.active").attr("value");
