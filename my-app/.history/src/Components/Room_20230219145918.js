...
class Room extends Component{
    ...
    putData = (pid) => {
        ...
        var url = "https://localhost:5001/api/v1/Rooms/" + pid;
        axios
        .put(url, {
        ...
        return this.state.data.map((item) => {
            return(
                <tr key={item.pid}>
                    ...
                    <td>{item.isDelete}</td>
                    <td>{item.moTa}</td>
                    <td><button type="button" className="btn btn-warning btn-sm" onClick={()=>this.RoomUpdateShowForm(item)}>Sửa</button></td>
                </tr>
            );
        });
   } 
