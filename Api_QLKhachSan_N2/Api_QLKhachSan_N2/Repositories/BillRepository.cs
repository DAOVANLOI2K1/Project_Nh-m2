...
namespace Api_QLKhachSan_N2.Repositories
{
    public class BillRepository : IBillRepository
    {
        ...
        public IEnumerable<CustomerResponse> getBill_Customer([FromQuery] string customerID)
        {
            // Kết nối DB
            using (SqlServerConnection = new SqlConnection(configuration.GetConnectionString("LOIDV")))
            {
        ...
        public IEnumerable<OrderRoomResponse> getBill_OrderRoom([FromQuery] string customerID)
        {
            // Kết nối DB
            using (SqlServerConnection = new SqlConnection(configuration.GetConnectionString("LOIDV")))
            {
        ...
        public IEnumerable<OrderServiceResponse> getBill_OrderService([FromQuery] string customerID)
        {
            // Kết nối DB
            using (SqlServerConnection = new SqlConnection(configuration.GetConnectionString("LOIDV")))
            {
        ...
        public double? getBill_Payment([FromQuery] string customerID)
        {
            // Kết nối DB
            using (SqlServerConnection = new SqlConnection(configuration.GetConnectionString("LOIDV")))
            {
                ...
                // Thực thi proc
                var result = SqlServerConnection.Execute(getProcedure, parameters, commandType: System.Data.CommandType.StoredProcedure);
                if (result != null)
                {
                    return result;
                }
